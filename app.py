...
def notifications():
    try:
        # Получаем уведомления с дополнительной информацией о сообщениях
        notifications_data = execute_query('''
            SELECT 
                ...
                n.is_read, 
                n.created_at,
                COALESCE(a.title, g.title) as property_title,  -- Берем название из соответствующей таблицы
            ...
            LEFT JOIN messages m ON 
                n.property_type = m.property_type AND 
                n.property_id = m.property_id AND
                m.sender_id != n.user_id
