...
def my_apartments():
    try:
        apartments_data = execute_query('''
            SELECT a.*, COUNT(m.id) as message_count
            FROM apartments a
            LEFT JOIN messages m ON a.id = m.property_id AND m.property_type = 'apartment'
            WHERE a.owner_id = %s
            GROUP BY a.id
        ''', (session['user_id'],))

        return render_template('my_apartments.html', apartments=apartments_data)
    except Exception as e:
        logger.error(f"My apartments error: {str(e)}")
        flash('Ошибка при загрузке ваших объявлений', 'danger')
        return redirect(url_for('index'))
...
def garages():
    try:
        ...
        flash('Ошибка при загрузке списка гаражей', 'danger')
        return redirect(url_for('index'))
    finally:
        cursor.close()
...
def create_garage():
    if request.method == 'POST':
        try:
            ...
            cursor.execute('''
                INSERT INTO garages 
                (title, description, price, area, address, district, owner_id, 
                 height, has_power, security, vehicle_type, created_at)
...
def my_garages():
    try:
        ...
        # Получаем гаражи пользователя с количеством сообщений
        cursor.execute('''
            SELECT g.*, COUNT(m.id) as messages_count 
            FROM garages g
            LEFT JOIN messages m ON g.id = m.property_id AND m.property_type = 'garage'
            WHERE g.owner_id = %s
            GROUP BY g.id
        ...
        return render_template('my_garages.html', garages=garages)
    except Exception as e:
        logger.error(f"My garages error: {str(e)}")
        flash('Ошибка при загрузке ваших гаражей', 'danger')
        return redirect(url_for('index'))
    finally:
        cursor.close()
